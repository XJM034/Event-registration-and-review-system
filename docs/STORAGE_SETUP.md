...
| `registration-files` | 队伍 Logo、报名附件 | 私有访问（经应用代理） | 10MB | JPG, PNG, WEBP, PDF |
| `player-photos` | 队员照片、证件照 | 私有访问（经应用代理） | 5MB | JPG, PNG, WEBP |
| `team-documents` | 队伍文档、自定义字段附件 | 私有访问（经应用代理） | 10MB | JPG, PNG, WEBP, PDF |
...
A:
- 只有 `event-posters` 需要公开访问，用于赛事海报展示
- `registration-files`、`player-photos`、`team-documents` 已改为私有 bucket，通过应用内 `/api/storage/object` 受控读取
...
1. **敏感 bucket 私有化**: `registration-files`、`player-photos`、`team-documents` 应保持 `public=false`

...
本文件承接从根 `CLAUDE.md` / `AGENTS.md` 下沉的实现细节，供 AI agent 按需读取。它描述当前仓库事实，不是目标蓝图；若与代码冲突，以代码和 `docs/sql/actual-supabase-schema.sql` 为准。
...
- `scripts/verify-security-posture.mjs` 连接真实 Supabase/MemFire 检查 Auth 设置、审计表和匿名隔离。
...
主要数据结构以 `docs/sql/actual-supabase-schema.sql` 为准。核心表：

- `admin_users`：管理员资料，含 `auth_id`、`phone`、`is_super`。
- `coaches`：教练资料，含 `auth_id`、手机号、学校/组织信息。
- `events`：赛事基本信息与可见性。
- `registration_settings`：动态报名配置，`team_requirements` / `player_requirements` 为 JSONB。
- `registrations`：报名记录，含 `team_data`、`players_data`、`status`、审核字段和取消字段。
- `notifications`：教练通知。
- `player_share_tokens`：公开队员填写 token、填写状态和最小同步数据。
- `security_audit_logs`：best-effort 安全审计日志。

关键数据库行为：

- `registration_notification_trigger` 会在报名状态更新时写通知。
...
- 管理端上传：`POST /api/upload`，需要管理员会话，使用 service role，默认 bucket 为 `event-posters`，含扩展名、MIME、文件签名和大小校验。
- 门户上传：`POST /api/portal/upload`，需要教练会话，使用 service role。
- 公开队员上传：`POST /api/player-share/[token]/upload`，无需登录，但会校验分享 token、报名状态、截止时间、bucket、文件签名、大小限制和 rate limit。
...
- 教练模板导出能力涉及 `lib/template-document-export.ts` 和 `app/api/portal/events/[id]/template-export/route.ts`。
...
- `pnpm security:check` 需要真实 Supabase/MemFire 环境，不能在缺少 env 时视为已验证。
...
- 发现旧文档与代码冲突时，不要只在聊天里说明；应更新对应文档或补“已知差异”。

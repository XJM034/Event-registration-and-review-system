...
**当前联调稳定账号**（用户确认不会变动）：
- 超级管理员：`18140044662` / `REDACTED_CURRENT_TEST_PASSWORD`
- 普通管理员：`15196653658` / `REDACTED_CURRENT_TEST_PASSWORD`
- 教练：`13800000001` / `REDACTED_CURRENT_TEST_PASSWORD`

**仓库 SQL 默认账号**（仅适用于执行初始化脚本后的全新环境）：
- 超级管理员：`18140044662` / `REDACTED_SQL_DEFAULT_PASSWORD`
- 超级管理员：`13164550100` / `REDACTED_SQL_DEFAULT_PASSWORD`
- 教练：`13800000001` ~ `13800000005` / `REDACTED_SQL_DEFAULT_PASSWORD`
...
| 角色 | 账号 | 密码 | 说明 |
|------|------|------|------|
| **超级管理员** | `18140044662` | `REDACTED_CURRENT_TEST_PASSWORD` | 完整权限，包括账号管理 |
| **普通管理员** | `15196653658` | `REDACTED_CURRENT_TEST_PASSWORD` | 管理端常规权限 |
| **教练** | `13800000001` | `REDACTED_CURRENT_TEST_PASSWORD` | 门户端权限 |
...
| 角色 | 账号 | 密码 | 说明 |
|------|------|------|------|
| **超级管理员** | `18140044662` | `REDACTED_SQL_DEFAULT_PASSWORD` | SQL 初始化默认账号 |
| **超级管理员** | `13164550100` | `REDACTED_SQL_DEFAULT_PASSWORD` | SQL 初始化默认账号 |
| **教练** | `13800000001` | `REDACTED_SQL_DEFAULT_PASSWORD` | SQL 初始化默认账号 |

### 其他测试账号

- 通过超级管理员在 `/admin/account-management` 创建账号或重置密码后，实际测试环境中的口令可能会变动
- `13800000002` ~ `13800000005` 仍是 SQL 初始化脚本中的默认教练账号，默认密码 `REDACTED_SQL_DEFAULT_PASSWORD`
- `/auth/login` 页面底部当前仍展示“默认密码：REDACTED_SQL_DEFAULT_PASSWORD（管理员）/ REDACTED_SQL_DEFAULT_PASSWORD（教练）”，这只是 SQL 初始化默认口令提示，不代表当前联调环境口令
...
5. **测试登录页文案与实际逻辑不一致，且页面不可访问**
   - 问题：
     - `app/test-login/page.tsx` 默认 state 使用的是 `REDACTED_SQL_DEFAULT_PASSWORD`
...
6. **统一登录页帮助文案仍写死 SQL 默认密码**
   - 现状：`app/auth/login/page.tsx` 底部提示仍显示 `REDACTED_SQL_DEFAULT_PASSWORD（管理员）/ REDACTED_SQL_DEFAULT_PASSWORD（教练）`

...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/auth/admin-session` | POST | 创建管理员会话（验证 Supabase session → 生成 JWT） |
...
- 生成链接：`app/portal/events/[id]/register/page.tsx`（会 insert 到 `player_share_tokens`）
  - 生成前会先把同一 `registration_id + player_id` 的旧 token 失效
  - 当前 token 格式：`${Date.now()}-${Math.random().toString(36).substring(2, 15)}`
...
- 队员分享页当前也会尝试复用 `/api/portal/upload`
  - 但该接口受 `/api/portal/*` 鉴权保护，匿名分享页上传会被拦截，见“已知不一致”
...
| `/api/admin/me` | GET | 获取当前管理员用户信息 |
| `/api/admin/me` | PUT | 更新当前管理员密码 |
...
| `/api/events/[id]/registrations/export` | POST | 导出报名压缩包（当前始终返回 zip） |
| `/api/events/[id]/divisions` | GET | 获取赛事分组 |
| `/api/events/[id]/divisions` | PUT | 更新赛事分组 |
| `/api/registrations/[id]` | GET | 获取单个报名记录（管理员） |
...
| `/api/admin/coaches/batch-status` | PATCH | 按筛选条件批量启用/禁用教练账号（需超级管理员） |
| `/api/admin/coaches/import` | POST | 从 Excel 批量导入教练账号（需超级管理员） |
...
| `/api/events/[id]/registration-settings/template-preview` | POST | 预览报名模板 |
...
| `/api/portal/registrations/[id]/template-export` | GET | 导出报名为 PDF（教练端） |
...
| 端点 | 方法 | 说明 |
|---|---|---|
| `/api/init-admin` | GET | 返回当前管理员列表并提示改用 SQL 脚本，不会真正初始化账号 |
...
4. **公开队员分享页的上传能力与路由保护冲突**
   - 现状：`app/player-share/[token]/page.tsx` 会调用 `/api/portal/upload`
   - 问题：`/api/portal/*` 在 `middleware.ts` 中要求教练登录态，而分享页本身是匿名可访问页面
   - 影响：匿名用户在分享页上传图片/附件时会收到 401/重定向
   - 修复方案：为分享页单独提供公开上传接口，或调整鉴权策略
